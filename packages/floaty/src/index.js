...
function createFloaty(_a) {
    ...
    var FLOATY = floaty.rawWindow("\n        <frame w=\"" + 2 * radius + "\" h=\"" + 2 * radius + "\">\n            " + items.map(function (item) {
        return "\n                <frame id=\"" + item.id + "\" w=\"" + size + "\" h=\"" + size + "\" alpha=\"0\" layout_gravity=\"center\">\n                    <img w=\"" + size + "\" h=\"" + size + "\" id=\"" + item.id + "_color\" src=\"" + (core_1.getPrototype(item.color) === 'String' ? item.color : item.color && item.color && item.color.length > 0 && item.color[0]) + "\" circle=\"true\" />\n                    <img w=\"" + ICON_SIZE + "\" h=\"" + ICON_SIZE + "\" id=\"" + item.id + "_icon\" src=\"@drawable/" + (core_1.getPrototype(item.icon) === 'String' ? item.icon : item.icon && item.icon.length > 0 && item.icon[0]) + "\" tint=\"" + (item.tint || '#ffffff') + "\" gravity=\"center\" layout_gravity=\"center\" />\n                </frame>\n                    ";
    ...
    }), operators_1.startWith(false), operators_1.shareReplay(1));
    ...
    function animation() {
        return new Promise(function (resolve, reject) {
            ...
            var direction = STAND.getX() < core_1.getWidthPixels() / 2 ? 1 : -1;
    ...
    items.forEach(function (item) {
        var index = 0;
        var iconLength = core_1.getPrototype(item.icon) === 'Array' && item.icon.length || 0;
        var colorLength = core_1.getPrototype(item.color) === 'Array' && item.color.length || 0;
